# MAI Studio 고객사 배포 가이드

MAI Studio를 고객사의 AWS EC2 서버(Amazon Linux 2023)에 배포하기 위한 단계별 가이드.

- **예시 도메인**: `mai-studio.lcampus.co.kr`
- **참조 배포**: `moai.magicecole.com` (매직에콜 운영 서버 — 동일 패턴)
- **대상 OS**: Amazon Linux 2023 (AL2023, Fedora 계열)
...
AL2023에서는 **AWS 보안 그룹이 1차 방화벽**입니다. OS 레벨 방화벽(firewalld)은 기본적으로 설치되지 않은 경우가 많습니다.

```bash
# firewalld 설치 여부 확인 (없으면 SG만으로 충분)
sudo systemctl status firewalld 2>/dev/null || echo "firewalld 미설치 — AWS SG만 사용"

# firewalld가 활성 상태이면 HTTP/HTTPS 허용
...
```bash
# 2-1. Docker 엔진 설치 (AWS 공식 저장소 사용)
sudo dnf install -y docker
sudo systemctl enable --now docker

# 2-2. 현재 사용자를 docker 그룹에 추가 (sudo 없이 docker 명령 실행 가능)
sudo usermod -aG docker $USER
# ⚠️ 그룹 변경 적용을 위해 로그아웃 후 재접속 필요

# 2-3. Docker Compose v2 플러그인 설치
...
AL2023에서는 `/etc/nginx/conf.d/*.conf`에 설정을 바로 둡니다 (sites-available/sites-enabled 구조 아님).
...
**AL2023은 기본 SELinux Enforcing 모드**입니다. 이 상태에서는 nginx가 로컬 127.0.0.1:8080으로 프록시 연결을 시도할 때 **차단됩니다** (`502 Bad Gateway`).
...
# 만약 "No match for argument" 에러가 나면 아래 pip 방식 사용
```

**pip 방식 (기본 저장소에 없을 경우)**:
...
**가장 흔한 원인**: SELinux가 nginx의 로컬 프록시를 차단.
...
**그 외**:
- Docker nginx 컨테이너가 다운되었는지 확인: `docker-compose ps`
- 8080 포트가 열려있는지: `sudo ss -tlnp | grep 8080`
- 로그 확인: `sudo tail -50 /var/log/nginx/error.log`

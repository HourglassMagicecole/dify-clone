...
### ⚠️ 중요 — `init-env.sh`가 `.env`를 `.env.example` 기반으로 동기화한다는 사실

`make docker-up`은 내부적으로 `docker/init-env.sh`를 호출합니다. 이 스크립트는 `.env`가 이미 있으면 `.env.example`을 기반으로 **동기화하면서 일부 키를 기본값으로 덮어씁니다**. 백업/복원 대상은 다음 13개로 한정됩니다.

```
SECRET_KEY, API_KEY_ENCRYPTION_KEY,
INITIAL_ADMIN_EMAIL / INITIAL_ADMIN_PASSWORD / INITIAL_ADMIN_NAME,
DB_PASSWORD, REDIS_PASSWORD, SANDBOX_API_KEY, CODE_EXECUTION_API_KEY,
PLUGIN_DAEMON_KEY, PLUGIN_DIFY_INNER_API_KEY,
ELASTICSEARCH_PASSWORD, CELERY_BROKER_URL
```

**`EXPOSE_NGINX_PORT`와 `EXPOSE_NGINX_SSL_PORT`는 이 목록에 없습니다.** 따라서 위 "사용자가 직접 수정해야 하는 값 (포트 2개만)" 안내에 따라 `.env`를 8080/8443으로 수정해도, `make docker-up` 실행 시점에 `.env.example`의 기본값(80/443)으로 **되돌아갑니다**.

**실전 권장 경로**: `make docker-up`으로 일단 스택을 기동한 뒤, 아래 "💡 포트 수정 누락 시 복구" 절차로 `.env` 포트를 8080/8443으로 고치고 nginx 컨테이너만 재생성하는 것이 현재 프로젝트에서 실제로 동작하는 흐름입니다.

**⚠️ 운영 주의**: 배포 완료 후 운영 중에 **`make docker-up`을 다시 실행하면** 포트가 다시 80/443으로 초기화되어 호스트 nginx와 포트 충돌이 납니다. 운영 중 재기동이 필요하면 `make docker-up` 대신 `docker-compose restart <서비스명>` 또는 `docker-compose up -d <서비스명>`(init-env.sh를 안 탐)을 사용하세요.

이 동작은 차기 Hotfix에서 `init-env.sh`에 포트 키 백업 추가 또는 `.env.example` 기본값 8080/8443 변경으로 근본 해결될 예정입니다.

### 💡 `make docker-up` 이후에 포트 수정이 누락된 걸 발견했다면

`docker-compose ps` 결과에서 nginx 컨테이너의 PORTS가 `0.0.0.0:80->80/tcp, ...:443->443/tcp` 로 보인다면 `.env` 포트 수정이 반영 안 된 상태입니다. 이대로 6단계 호스트 nginx 설치로 가면 **호스트 80/443 포트 충돌로 실패**합니다. 전체 스택을 다시 올릴 필요 없이 nginx 컨테이너만 재생성하면 됩니다.

```bash
# 1) 현재 값 확인
grep -E "EXPOSE_NGINX_PORT|EXPOSE_NGINX_SSL_PORT" /opt/mai-studio/docker/.env
...
# 4) 확인 — 0.0.0.0:8080->80/tcp 로 바뀌어야 함
docker-compose ps nginx
curl -I http://127.0.0.1:8080   # → 307 Temporary Redirect
curl -I http://127.0.0.1        # → curl: (7) Failed to connect (호스트 nginx가 쓸 자리 비워둠)
```

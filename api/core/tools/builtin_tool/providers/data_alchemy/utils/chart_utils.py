    ...
    def normalize_data(
        ...
        for col in columns:
            match method:
                ...
                case _:
                    # Unknown method, skip normalization for this column
                    pass

        return result

    @staticmethod
    def format_datetime(data: pd.DataFrame, datetime_columns: list[str], format: str = "%Y-%m-%d") -> pd.DataFrame:
        """
        格式化日期时间
        :param data: 输入的DataFrame
    ...
    def generate_basic_chart(
        ...
        match chart_type:
            case "bar" | "line":
                ...
                base_option.xAxis = {"type": "category", "data": chart_data.x_data}
                base_option.yAxis = {"type": "value"}
                base_option.series = [
                    {"name": series.name, "type": chart_type, "data": series.data} for series in chart_data.series
                ]

            case "pie":
                pie_data = data if isinstance(data, PieData) else PieData(name="", data=[])
                base_option.series = [{"name": pie_data.name, "type": "pie", "radius": "50%", "data": pie_data.data}]

            case _:
                # Unknown chart type, return base option with empty series
                pass
...
if __name__ == "__main__":
    # 数据处理示例
    data = pd.DataFrame(
        {"date": ["2023-01-01", "2023-01-02", "2023-01-03"], "value": [100, np.nan, 300], "category": ["A", "B", "C"]}
    )
    ...
    chart_data = ChartData(x_data=cleaned_data["date"].tolist(), series=[series_data])

    basic_chart = chart_gen.generate_basic_chart(
        chart_type="bar", data=chart_data, title="示例图表", subtitle="数据展示"
    )
    ...
    custom_config = ChartConfig(
        ...
        legend={"data": []},
        xAxis={"type": "category", "data": cleaned_data["date"].tolist()},
        yAxis={"type": "value"},
        series=[{"type": "bar", "data": cleaned_data["value"].tolist()}],
    )

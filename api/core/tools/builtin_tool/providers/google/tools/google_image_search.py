...
class GoogleImageSearchTool(BuiltinTool):
    ...
    def _invoke(
        ...
        """
        Invoke Google Image Search tool.

        Args:
            user_id: User ID
            tool_parameters: Tool parameters with 'query', 'hl', 'gl', etc.
        ...
        if hl not in VALID_LANGUAGES:
            yield self.create_text_message(
                f"Invalid 'hl' parameter: {hl}. "
                f"Please refer to https://serpapi.com/google-languages for a list of valid language codes."
            )
            return
        ...
        if gl not in VALID_COUNTRIES:
            yield self.create_text_message(
                f"Invalid 'gl' parameter: {gl}. "
                f"Please refer to https://serpapi.com/google-countries for a list of valid country codes."
            )
            return
        ...
        try:
            response = requests.get(url=SERP_API_URL, params=params, timeout=30)
            ...
            yield self.create_text_message(
                ...
                f"Please refer to https://serpapi.com/locations-api for the list of valid locations."
            )

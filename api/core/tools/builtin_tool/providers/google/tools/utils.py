    ...
    def model_post_init(self, context: Any, /) -> None:
        if not self.site_name and self.url:
            try:
                ...
                # Silently skip URL parsing errors
                pass
...
def load_valid_countries(filepath: Path) -> set | None:
    ...
    Args:
        filepath: Path to the google-countries.json file

    Returns:
        Set of valid country codes or None if failed
    """
    try:
...
def load_valid_languages(filepath: Path) -> set | None:
    ...
    Args:
        filepath: Path to the google-languages.json file

    Returns:
        Set of valid language codes or None if failed
    """
    try:
...
def to_refs(response: dict) -> list[SearchRef]:
    """
    Convert search response to list of SearchRef objects.

    Args:
        response: Search response dictionary from SerpApi

    Returns:
        List of SearchRef objects
    """
    refs = []
...
VALID_COUNTRIES: set[str] | None = load_valid_countries(PROJECT_PATH.joinpath("google-countries.json"))
VALID_LANGUAGES: set[str] | None = load_valid_languages(PROJECT_PATH.joinpath("google-languages.json"))

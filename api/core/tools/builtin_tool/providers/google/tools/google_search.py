...
from core.tools.builtin_tool.providers.google.tools.utils import (
    VALID_COUNTRIES,
    VALID_LANGUAGES,
...
class GoogleSearchTool(BuiltinTool):
    ...
    @staticmethod
    def _set_params_language_code(params: dict, tool_parameters: dict) -> None:
        """
        Set language code parameter if valid.

        Args:
            params: Request parameters dictionary
            tool_parameters: Tool parameters from user
        """
        try:
            ...
            # Silently skip invalid language code
            pass

    @staticmethod
    def _set_params_country_code(params: dict, tool_parameters: dict) -> None:
        """
        Set country code parameter if valid.
        ...
        try:
            ...
            # Silently skip invalid country code
            pass
    ...
    def _invoke(
        ...
        """
        Invoke Google search tool.

        Args:
            user_id: User ID
            tool_parameters: Tool parameters with 'query', 'language_code', 'country_code', etc.
        ...
        try:
            response = requests.get(url=self.SERP_API_URL, params=params, timeout=30)
            ...
            if not as_agent_tool:
                ...
                yield self.create_json_message(isr_json_parts)
            else:
                yield self.create_text_message(isr.to_dify_text_message())

...
def getParagraphs(slide, wantedParagraphs=[]):
    ...
    for theShape in slide.shapes:
        if (theShape.shape_type == MSO_SHAPE_TYPE.TEXT_BOX) | (
            theShape.placeholder_format.type == PP_PLACEHOLDER.OBJECT
        ):
...
def parseText(text):
    ...
    text2 = re.sub(globals.spanStyleRegex, "\ufdd1", text2)

    # Replace any span class starts with char "\uFDD2"
    text2 = re.sub(globals.spanClassRegex, "\ufdd2", text2)

    # Replace any span ends with char "\uFDD3"
    text2 = text2.replace("</span>", "\ufdd3")

    # Replace any abbreviation starts with char "\uFDD4"
    text2 = text2.replace("<abbr title=", "\ufdd4")

    # Replace any abbreviation ends with char "\uFDD5"
    text2 = text2.replace("</abbr>", "\ufdd5")
    ...
    text2 = text2.replace("<ins>", "\ufddd")

    # Replace any </ins> with char "\uFDDD"
    text2 = text2.replace("</ins>", "\ufddd")

    # Replace any <del> with char "\uFDDE"
    text2 = text2.replace("<del>", "\ufdde")

    # Replace any </del> with char "\uFDDE"
    text2 = text2.replace("</del>", "\ufdde")

    # Replace any <sub> with char "\uFDDF"
    text2 = text2.replace("<sub>", "\ufddf")

    # Replace any </sub> with char "\uFDDF"
    text2 = text2.replace("</sub>", "\ufddf")

    # Replace any <sup> with char "\uFDE0"
    text2 = text2.replace("<sup>", "\ufde0")
        ...
        if c == "*":
            ...
            if state == "LinkText1":
                ...
                if foundReference:
                    ...
                    textArray.append(["Link", indLinkText + "\ufdd8" + indURL])
                else:
...
def addFormattedText(p, text):
    ...
    for f in range(len(parsedText)):
        if parsedText[f][0] in ["SpanClass", "SpanStyle"]:
            parsedText[f][1][1] = parsedText[f][1][1].replace("\ufde2", ">")
        else:
            parsedText[f][-1] = parsedText[f][-1].replace("\ufde2", ">")

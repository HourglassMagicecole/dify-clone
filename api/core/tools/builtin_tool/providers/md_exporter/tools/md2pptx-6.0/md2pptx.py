...
def addMissingMediaPlaceholder(slide, mediaFilename, left, top, width, height):
    ...
    missingRectangle = slide.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, adjustedLeft, adjustedTop, adjustedWidth, adjustedHeight
    )
...
def createCodeBlock(slideInfo, slide, renderingRectangle, codeBlockNumber):
    ...
    fill = codeBox.fill
    fill.solid()
    fill.fore_color.rgb = RGBColor.from_string(globals.processingOptions.getCurrentOption("codeBackground"))
    ...
    fixedPitchHeightWidthRatio = globals.processingOptions.getCurrentOption("fixedPitchHeightWidthRatio")
    ...
    p.font.color.rgb = RGBColor.from_string(globals.processingOptions.getCurrentOption("codeforeground"))
        ...
        for c, card in enumerate(slideInfo.cards):
            ...
            if card.graphic != "":
                ...
                try:
                    ...
                    print("Missing card image file: " + card.printableFilename)
                ...
                try:
                    ...
                    print("Missing card media file: " + card.printableFilename)
        ...
        if (cardGraphicShape == None) & (cardMediaShape == None):
            ...
            if cardGraphicPosition == "before":
                ...
                if cardLayout == "horizontal":
                    ...
                    if cardMediaShape is not None:
                        ...
                        cardGraphicShape.top = cardBodyShape.top + int((cardBodyHeight - cardMediaHeight) / 2)
                ...
                if cardLayout == "horizontal":
                    if cardMediaShape is not None:
                        ...
                        cardGraphicShape.top = cardBodyShape.height + cardBodyShape.top + cardGraphicPadding

                else:
                    cardGraphicShape.top = cardBodyShape.top + int((cardBodyHeight - cardGraphicSizeRaw) / 2)
            ...
            if cardLayout == "horizontal":
                if cardMediaShape is not None:
                    ...
                    cardGraphicShape.left = cardTitleShape.left + int((cardTitleShape.width - cardMediaWidth) / 2)
                ...
                if cardGraphicPosition == "before":
                    ...
                    if cardMediaShape is not None:
                        ...
                        cardGraphicShape.left = card.left + cardBodyShape.width + cardGraphicPadding
...
def addSlideTransition(slide, transitionType):
    ...
    elif transitionType == "push":
        xml += "         <p:" + transitionType + ' dir="u"/>\n'

    elif transitionType == "vortex":
        xml += "         <p14:" + transitionType + ' dir="r"/>\n'

    elif transitionType == "split":
        xml += "         <p:" + transitionType + ' orient="vert"/>\n'

    elif transitionType == "fracture":
...
# Footnotes defaults
globals.processingOptions.setOptionValuesArray([["footnotesTitle", "Footnotes"], ["footnotesPerPage", 20]])
...
globals.processingOptions.setOptionValuesArray(
    [["applescript", ""], ["applescriptReload", False], ["applescriptRun", False], ["bookmark", 0]]
)
...
for line in metadata_lines:
    ...
    if name == "numbers":
    ...
    elif name == "numbersfontsize" or name == "pagetitlesize":
    ...
    elif name == "basetextsize" or name == "basetextdecrement":
        globals.processingOptions.setOptionValues(name, float(value))

    elif name == "backgroundimage" or name in [
        "onpresentationinitialisation",
        "onpresentationbeforesave",
        "onpresentationaftersave",
    ]:
    ...
    elif name == "deletefirstslide" or name == "hidden" or name == "sectionsexpand":
        if value.lower() == "yes":
    ...
    elif name == "marginbase" or name == "tablemargin":
    ...
    elif name == "toclinks" or name == "sectionarrows":
        if value.lower() == "yes":
            globals.processingOptions.setOptionValues(name, True)
    ...
    elif name == "tocrowgap" or name == "tocfontsize" or name == "compacttables" or name == "tableheadingsize":
        globals.processingOptions.setOptionValues(name, float(value))
    ...
    elif name == "abstracttitle" or name in [
        "leftfootertext",
        "middlefootertext",
        "rightfootertext",
        "sectionfooters",
        ...
        globals.processingOptions.setOptionValues("cardBorderColour", parseColour(value.strip()))
        ...
        globals.processingOptions.setOptionValues("cardDividerColour", parseColour(value.strip()))

    elif name == "cardborderwidth" or name == "cardtitlesize":
        globals.processingOptions.setOptionValues(name, float(value))
        ...
        if value in ["above", "inside", "before", "after"]:
            ...
            print(f'CardTitlePosition value \'{value}\' unsupported. "inside", "above", "before", or "after" required.')
    ...
    elif name == "tasksperpage" or name in [
        "titleslidelayout",
        "sectionslidelayout",
        "contentslidelayout",
        "titleonlylayout",
    ...
    elif name == "glossarytitle" or name == "glossaryterm" or name == "glossarymeaning":
        globals.processingOptions.setOptionValues(name, value)

    elif name == "glossarymeaningwidth" or name == "glossarytermsperpage" or name == "footnotesperpage":
        globals.processingOptions.setOptionValues(name, int(value))
    ...
    # Following relate to styling and don't use Processing Options class
        ...
        globals.processingOptions.setOptionValues("funnelBorderColour", parseColour(value.strip()))

    elif (name == "funneltitlecolour") | (name == "funneltitlecolor"):
        globals.processingOptions.setOptionValues("funnelTitleColour", parseColour(value.strip()))

    elif (name == "funneltextcolour") | (name == "funneltextcolor"):
        globals.processingOptions.setOptionValues("funnelTextColour", parseColour(value.strip()))
        ...
        if value in ["left", "right", "pipe", "hpipe", "vpipe", "top", "bottom"]:
            ...
            print(f'funnelLabelPosition value \'{value}\' unsupported. "left", "right", "pipe", or "hpipe" required.')
        ...
        if DMMatch := dynamicMetadata2Regex.match(line):
            ...
            if (metadataKey != "") & (metadataValue != ""):
                ...
                if (metadataKey == "pagesubtitlesize") & (metadataValue == "same"):
                    ...
                    globals.processingOptions.dynamicallySetOption(metadataKey, metadataValue, "")
                ...
                # Floating point values where metadata key matches directly
                    ...
                    if metadataValue == "default":
                        numbersheight = globals.processingOptions.getDefaultOption("numbersHeight")
                    elif metadataValue == "pres":
                        numbersheight = globals.processingOptions.getPresentationOption("numbersHeight")
                    else:
                        numbersheight = int(Inches(float(metadataValue)))

                    globals.processingOptions.dynamicallySetOption(metadataKey, numbersheight, "int")
                    ...
                    if metadataValue == "default":
                        ...
                        marginbase = globals.processingOptions.getPresentationOption("marginbase")
                    else:
                        marginbase = int(Inches(float(metadataValue)))

                    globals.processingOptions.dynamicallySetOption(metadataKey, marginbase, "int")
                    ...
                    if metadataValue == "default":
                        ...
                        tablemargin = globals.processingOptions.getPresentationOption("tablemargin")
                    else:
                        tablemargin = int(Inches(float(metadataValue)))

                    globals.processingOptions.dynamicallySetOption(metadataKey, tablemargin, "int")
                ...
                # Note: Actual value handling prevents using dynamicallySetOption
                    ...
                    globals.processingOptions.dynamicallySetOption(metadataKey, metadataValue.lower(), "")
                    ...
                    globals.processingOptions.dynamicallySetOption("cardColour", valueArray2, "")
                    ...
                    globals.processingOptions.dynamicallySetOption("cardTitleBackground", valueArray2, "")

                elif metadataKey in ["funnelcolours", "funnelcolors"]:
                    valueArray2 = [parseColour(c.strip()) for c in metadataValue.split(",")]

                    globals.processingOptions.dynamicallySetOption("funnelColours", valueArray2, "")
                ...
                # Note: Actual value handling prevents using dynamicallySetOption
                    ...
                    if metadataValue == "default":
                        contentSplit = globals.processingOptions.getDefaultOption("contentSplit")
                    elif metadataValue == "pres":
                        contentSplit = globals.processingOptions.getPresentationOption("contentSplit")
                    ...
                    globals.processingOptions.dynamicallySetOption("contentSplit", contentSplit, "")
...
if len(globals.footnoteDefinitions) > 0:
    slideNumber, footnoteSlides = createFootnoteSlides(prs, slideNumber, globals.footnoteDefinitions)
...
if len(globals.abbrevDictionary) > 0:
    glossaryTermsPerPage = globals.processingOptions.getCurrentOption("glossaryTermsPerPage")
    slideNumber, glossarySlides = createGlossarySlides(prs, slideNumber, globals.abbrevDictionary)
...
if (len(tasks) > 0) & (taskSlides != "none"):
    ...
    if taskSlides != "all":
        ...
        if (
            (taskSlides == "separate") & (len(tasks) > 0)
            or (taskSlides == "remaining") & (len(incomplete) > 0)
            or (taskSlides == "done") & (len(complete) > 0)
        ):
...
if applescriptFilename != "":
    ...
    if want_reload:
        ...
        globals.applescriptLines.append(f'    open "{full_filename}"')
    ...
    globals.applescriptLines.append("end tell")

...
import type { DeleteProgress, DeleteResult } from '@/types/session'
...
interface ResourceDeleteDialogProps {
  ...
  accountId?: string
  accountName?: string
  resourceSummary: { total_apps: number, total_datasets: number }
  onClose: () => void
...
export function ResourceDeleteDialog({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const status = await sessionAPI.getDeleteStatus(sessionId, taskId)
        ...
        if (status.status === 'completed') {
          ...
          setPhase('failed')
          setError(status.error || t('delete_failed'))
          clearInterval(interval)
        }
  ...
  }, [phase, taskId, sessionId, t])

  const handleDelete = useCallback(async () => {
    try {
      setPhase('deleting')
      const response = await sessionAPI.deleteSessionResources(sessionId, accountId)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-6">
        ...
        {phase === 'confirm' && (
          <div>
            ...
            <div className="bg-gray-100 p-4 rounded mb-4">
              <p className="font-medium mb-2">{t('resources_to_delete')}:</p>
              <ul className="list-disc list-inside text-sm text-gray-600">
                <li>
                  {t('agents')}
                  :
                  {' '}
                  {resourceSummary.total_apps}
                </li>
                <li>
                  {t('datasets')}
                  :
                  {' '}
                  {resourceSummary.total_datasets}
                </li>
              </ul>
              {accountName && (
        ...
        {phase === 'deleting' && (
          <div>
            <div className="mb-4">
              <div className="text-lg font-bold">{t('deleting_resources')}</div>
              {progress && (
